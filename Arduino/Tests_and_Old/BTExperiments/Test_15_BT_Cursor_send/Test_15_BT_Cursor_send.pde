/* BT cursors from Nexus 4 -> Arduion/Linvor2 running BT_Display_incoming_arduino 
 * works PERFECTLY !!!
 * To run:
...
void doMyMessage(){
  ...
  int nbChars =  mx.length()+my.length()+4;
  byte[] posVec = new byte[nbChars];
  
  int pos = 0;
  posVec[pos++] = '(';
  for (int i=0; i<mx.length();i++){
     posVec[pos++] = byte(mx.charAt(i));
  }
  posVec[mx.length()] = ',';
  for (int i=0; i<my.length();i++){
     posVec[pos++] = byte(my.charAt(i));
  }
  posVec[pos++] = byte(')');
  posVec[pos++] =byte('\n');
